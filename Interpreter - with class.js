  ...
  function interpBlock(test_result, stated_value, program, isWhile, variables_name){// take a while loop body 
    ...
    while(test_result){// when test_result is true, executing body
      ...
      function loop_parts(program){
        ...
        let original = variables_name;
        while(variables_name.next !== undefined){
          let key_state = Object.getOwnPropertyNames(variables_name.current)
          for (let j = 0; j < key_state.length; ++j){
            if(lib220.getProperty(inner_stated_value, key_state[j]).found){
              let modified_value = lib220.getProperty(inner_stated_value, key_state[j]).value;
              lib220.setProperty(stated_value, key_state[j], modified_value);
            }  
          }  
          variables_name = variables_name.next;
        }
        variables_name = original;
      }
